...
def test_category_initialization(sample_category: Category) -> None:
    ...
    assert (
        len(sample_category.products) == 0
    )  # Проверяем, что список товаров пуст в начале


def test_category_and_product_count() -> None:
    ...
    product1 = Product(
        name="Молоко", description="Коровье молоко", price=1.5, quantity=100
    )
    product2 = Product(
...
def test_add_product_to_category(
    ...
    assert (
        sample_category.products[0] == sample_product
